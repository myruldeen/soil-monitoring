...
void readRS485();
void readAHT_LightSensor();
void sendMQTT(float lux, float tempv, float humidity, float moisture, float temperature, float ec, float ph, float nitrogen, float phosphorus, float potassium);
void displayResult(float tempv, float humidity, float lux, float moisture, float temperature, float ec, float ph, float nitrogen, float phosphorus, float potassium);
...
#error only ESP8266 and ESP32 reboot supported
  ...
  // for ESP-01 use aht10.begin(0, 2);
  while (aht10.begin() != true) { Serial.println(F("AHT1x not connected or fail to load calibration coefficient"));  delay(5000); }
  ...
  setup_wifi();
  Serial.print("RSSI (WiFi Signal Strength): ");
...
void loop() {
  if (rebootDelay.justFinished()) while (1){} // force watch dog timer reboot
  // Reconnecting.......
  if (!client.connected()) reconnect();

  if (!client.loop()) client.connect("ESP32Soil/soil_sensors", mqtt_user, mqtt_pass);
  ...
  if (now - lastMeasure>=10000) {
    ...
    readRS485();
    readAHT_LightSensor();
    sendMQTT(lux, tempv, humidity, moisture, temperature, ec, ph, nitrogen, phosphorus, potassium);
    displayResult(tempv, humidity, lux, moisture, temperature, ec, ph, nitrogen, phosphorus, potassium);
...
void sendMQTT(float lux, float tempv, float humidity, float moisture, float temperature, float ec, float ph, float nitrogen, float phosphorus, float potassium) {
  ...
  JsonObject &root = JSONbuffer.createObject();
    ...
    //    root.printTo(Serial);
  if (client.connect("ESP32Soil/soil_sensors", mqtt_user, mqtt_pass)) {
    ...
    // client.publish("ESP32Soil/soil_sensors", "Hello from ESP32");
    // client.subscribe("ESP32Soil/soil_sensors");
  } else {
